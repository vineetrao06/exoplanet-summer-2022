...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 20 17:40:09 2022
...
for i in range(0,len(df)):
    ...
    # for its orbital period and the star's mass, we can use
    # Kepler's 3rd Law to calculate its semi-major axis, and then 
    # check to determine if it is in the CHZ
    
    elif ((pd.isna(df['pl_orbsmax'][i])) & (pd.notna(df['pl_orbper'][i])) & (pd.notna(df['st_mass'][i]))):
